...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-default'>
			<li class='active'><a href='#'><h1><b>Categories</b></h1></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "
					<li><a href='#' class='category' cid='$cid'><h4><b>$cat_name/</b></h4></a></li>
			";
		}
		echo "</div>";
	}
...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-default'>
			<li class='active'><a href='#'><h1><b>Brands</b></h1></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
			...
			echo "
					<li><a href='#' class='selectBrand' bid='$bid'><h4><b>$brand_name</b></h4></a></li>
			";
		}
		echo "</div>";
	}
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-default'>
								...
								<div class='panel-heading'><h2>$.$pro_price.00</h2>
									<button pid='$pro_id' style='float:right width:10%' id='product' class='btn btn-danger btn-xs'><span class='glyphicon glyphicon-shopping-cart'></span><h6>Add To My Cart</h6></button>
								</div>
							</div>
						</div>	
			";
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				echo '
					<div class="row">
						...
						<div class="col-md-3">
						<div class="btn-group">
						<a href="#" remove_id="'.$product_id.'" class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span></a>
						</div>
						</div>
					</div>';
			...
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="payment_creditcard.php" method="post">
							<input type="hidden" name="cmd" value="_cart">
							<input type="hidden" name="business" value="shoppingcart@mycartshop.lk">
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/mycartshop.lk/mycartshop.lk/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/mycartshop.lk/mycartshop.lk/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/mycartshop.lk/mycartshop.lk/cancel.php"/>
									<input type="hidden" name="currency_code" value="USD"/>
									<input type="hidden" name="custom" value="'.$_SESSION["uid"].'"/>
									<input type="submit" style="float:right; margin-right:80px; background-color:#0099ff; width:160ppx; height:59px; color:white; font-weight:bold; font-size:30px;"  name="submit" value="Checkout Credit Card">
								</form>';
						echo '
